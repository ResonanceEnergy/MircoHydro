#!/usr/bin/env python3
"""MicroHydroV1 - Parameter optimizer (resonance + golden ratio heuristic).

LOCKED-IN resonance metric (v0.3.0):
  Primary resonance metric = T-002 hydraulic ripple spectral peakiness
  - Compute PSD on a steady-state segment of the ripple signal
  - Score = max(PSD) / median(PSD) in band [0.2, 20] Hz (lower is better)

This is a heuristic optimizer:
- Golden ratio (phi) is treated as a design heuristic, not a physics guarantee.
- Resonance metric comes from measured time-series when available.

Usage (phi-only):
  python tools/optimize/optimize_params.py --params cad/params/params.json --config tools/optimize/opt_config.json --out cad/params/params_optimized.json

Usage (phi + resonance peakiness):
  python tools/optimize/optimize_params.py     --params cad/params/params.json     --config tools/optimize/opt_config.json     --out cad/params/params_optimized.json     --resonance-csv tests/raw/<run>/T002_timeseries.csv     --signal-col Pressure_Pa --fs 100

CSV requirement:
- Must contain a numeric signal column (Pressure_Pa or Value)
- Provide sampling rate via --fs OR a time column in seconds via --time-col
"""
...
def read_timeseries_csv(path: Path, signal_col: str, time_col: Optional[str] = None):
    import csv
    xs = []
    ts = []
    with path.open('r', newline='', encoding='utf-8-sig') as f:
        r = csv.DictReader(f)
        for row in r:
            if signal_col not in row:
                raise ValueError(f"Signal column '{signal_col}' not found. Available: {r.fieldnames}")
            x = safe_float(row[signal_col])
...
def psd_peakiness(x: np.ndarray, fs: float, f_low: float, f_high: float, normalize: str = 'median') -> float:
...
def resonance_from_data(csv_path: Path, res_cfg: Dict[str, Any], fs: Optional[float], signal_col: str, time_col: Optional[str]) -> float:
    band = res_cfg.get('band_hz', [0.2, 20.0])
    f_low, f_high = float(band[0]), float(band[1])
    normalize = res_cfg.get('normalize', 'median')

    t, x = read_timeseries_csv(csv_path, signal_col=signal_col, time_col=time_col)
    ...
    return psd_peakiness(x, fs=fs_use, f_low=f_low, f_high=f_high, normalize=normalize)


def score(params: Dict[str, Any], cfg: Dict[str, Any], resonance_score: Optional[float]) -> Tuple[float, Dict[str, float]]:
    phi_err = 0.0
    ...
    res_err = 0.0
    if res_cfg.get('enabled', False) and resonance_score is not None:
        # penalize peakiness above 1.0
    ...
    w_res = float(cfg.get('weights', {}).get('resonance', 1.0))
    w_pen = float(cfg.get('weights', {}).get('penalty', 1.0))

    total = w_phi * phi_err + w_res * res_err + w_pen * penalty
    return total, {'phi_err': phi_err, 'res_err': res_err, 'penalty': penalty}


def propose(base: Dict[str, Any], bounds: Dict[str, List[float]], step_scale: float) -> Dict[str, Any]:
...
def main(argv=None):
    ...
    ap.add_argument('--signal-col', default='Value')
    ...
    best['Meta']['Optimization'] = {
        ...
        'resonance_score_input': resonance_score,
